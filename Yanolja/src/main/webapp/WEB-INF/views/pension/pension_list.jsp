      ...
      <div class="container">
         <div class="row">
            <div class="title-inner">
               ...
               <div class="title">
               <c:choose>
	               <c:when test="${type eq 'pseoul'}" >
	                  <h2>서울 펜션 리스트</h2>               
	               </c:when>
	               <c:when test="${type eq 'pkangwon'}" >
	                  <h2>강원 펜션 리스트</h2>   
	               </c:when>
	               
	               <c:when test="${type eq 'pbusan'}" >
	                  <h2>부산 펜션 리스트</h2>   
	               </c:when>
	               
	               <c:when test="${type eq 'pjeju'}" >
	                  <h2>제주 펜션 리스트</h2>   
	               </c:when>
               </c:choose>
         ...
         $.getJSON(
            ...
            function(data){
               ...
               for(var i = 0; i < data.length; i++) {
                  ...
                  str += '';
                  str += '<a href="${pageContext.request.contextPath}/pension/detail?pno=' + data[i].p_no + '">';
                  str += '<img src="${pageContext.request.contextPath}/resources/images/pension/' + data[i].p_no + '.jpg"></a>';
                  str += '</div>';
                  str += '<div class="content-inner">';
                  str += '<a href="${pageContext.request.contextPath}/pension/detail?pno=' + data[i].p_no + '"><h2>' + data[i].p_name + '</h2></a>';
                  str += '<p>숙박:<small>15:00~</small></p>';
                  str += '<h4><strong>' + comma(data[i].price) + '원 </strong></h4>';
                  str += '</div>';
                  str += '<div class="like-inner">';
                  str += '<span>'+ data[i].p_locabasic + '&nbsp;' + data[i].p_locadetail + '</span>';
                  str += '</div>';
               } // for 끝
    	         ...
    	         $.getJSON(
    	            ...
    	            function(data){
    	               ...
    	               for(var i = 0; i < data.length; i++) {
    	                  ...
    	                  str1 += '';
    	                  str1 += '<a href="${pageContext.request.contextPath}/pension/detail?pno=' + data[i].p_no + '">';
    	                  str1 += '<img src="${pageContext.request.contextPath}/resources/images/pension/'+ data[i].p_no +'.jpg"></a>';
    	                  str1 += '</div>';
    	                  str1 += '<div class="content-inner">';
    	                  str1 += '<a href="${pageContext.request.contextPath}/pension/detail?pno=' + data[i].p_no + '"><h2>' + data[i].p_name + '</h2></a>';
    	                  str1 += '<p>숙박:<small>15:00~</small></p>';
    	                  str1 += '<h4><strong>' + comma(data[i].price) + '원 </strong></h4>';
    	                  str1 += '</div>';
    	                  str1 += '<div class="like-inner">';
    	                  str1 += '<span>'+ data[i].p_locabasic + '&nbsp;' + data[i].p_locadetail + '</span>';
    	                  str1 += '</div>';
    	               } // for 끝
