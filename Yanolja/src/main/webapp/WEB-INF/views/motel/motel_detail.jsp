...
<body>
      ...
      <div class="container">
         <div class="row">
            <div class="title-inner">
               ...
               <!--제목영역-->
               <div class="title">
                  <img src="${pageContext.request.contextPath}/resources/images/motel/${mInfo.m_no }.jpg" width="70%">
                  <h2>${mInfo.m_name }</h2>
                  <p>${mInfo.m_locabasic}${mInfo.m_locadetail }</p>
               </div>
            </div>

            <!--반복-->
            <c:forEach var="vo" items="${mInfo.roominfo }">
               <div class="col-xs-12 col-sm-8 section-inner">
                  <div id="contentDiv">
                     <div class="image-inner">

                        <!-- 이미지영역 -->
                        <a href="${vo.mr_no }" data-toggle="modal" data-target="#myModal">
                        <img src="${pageContext.request.contextPath}/resources/images/motel/${mInfo.m_no }-${vo.mr_no }.jpg"></a>
                     </div>
                     <div class="content-inner">

                        <!--내용영역-->
                        <a href="#" data-toggle="modal" data-target="#myModal"><h2>${vo.mr_name }</h2></a>
                        <p class="ss">기준2인 / 최대${vo.m_per }인</p>
                        <br>
                        <div class="p-content">
                           <div class="p-left">
                              <p>
                           ...
                           <div class="p-rigth">
                              <h4>
                                 <strong><fmt:formatNumber>${vo.m_price }</fmt:formatNumber>원</strong>
                              </h4>
                              <button name="${vo.mr_no }" id="${mInfo.m_no }" type="button" class="tribtn" data-toggle="modal" data-target="#myModal">예약</button>
                           </div>
                        </div>
                     </div>
                  </div>
         ...
         <div class="modal-dialog">
            ...
            <div class="modal-content">
               <div class="modal-header">
                  ...
                  <h4 class="modal-title">${mInfo.m_name }</h4>
               </div>
               <div class="modal-body">
                  <img src="${pageContext.request.contextPath}/resources/images/motel/${mInfo.m_no }.jpg" class="innimage">
                  <div class="InOut">
                     <span class="checkIn"><small>체크인</small></span>
                     <span class="checkOut"><small>체크아웃</small></span>
                     <p>
                        <input type="text" id="mr_CheckIn" size="25" onChange="handleChange()">
                        <input type="text" id="mr_CheckOut" size="25" onChange="handleChange()">
                     </p>
                  </div>
                  <div id="AllPerson">
                     <div id="pperson">
                        <p id="person">인원수</p>
                        <select id="mr_Book_Per">
                           <option>1명</option>
                           <option>2명</option>
                           <option>3명</option>
                           <option>4명</option>
                     ...
                     <input type="password" id="mr_Book_Pw">
                  </div>
                  <div class="room">
                     <div class="title">
                        <h4>
   ...
   <!-- Modal 달력 -->
    <script>
      $(function() {
         ...
         var from = $("#mr_CheckIn").datepicker(optionFrom).on("change", function() {
                  to.datepicker("option", "minDate", getDate(this));
               });
         var optionTo = option;
         optionTo.altField = "#alternateTo";
         // 종료일이 선택이 되면 시작일은 시작일 보다 앞을 선택할 수 없다.
         var to = $("#mr_CheckOut").datepicker(optionTo).on("change", function() {
   ...
   <!-- =====================후기 & 별점======================= -->
    <script type="text/javascript">
    	...
    	$(document).ready(function() {
    		...
    		function add() {
				...
				$.ajax({
					type:"POST", 
					url:"../reply/motel_input_reply",
					data:JSON.stringify({"m_no":m_no,"rating":idx, "reply":reply, "replyId":replyId}),//전송 데이터
	...
	function handleChange() {
		$.getJSON(
			'getPrice/' + mr_no + '/' + mno, // 요청보낼 주소
			function(data){
	            var realmoney = data;
	            var date_out = new Date($("#mr_CheckOut").val());
	            var date_in = new Date($("#mr_CheckIn").val());
   ...
   /* 예약 함수 */
   function bookReserv(mr_no) {
      $(".bookbtn").click(function(){
         var bookpw = $("#mr_Book_Pw").val();
            ...
            if($("#mr_Book_Pw").val()==''){
                ...
                return false;
                  ...
                  return false;
               } else if ($("#mr_CheckIn").val() == $("#mr_CheckOut").val() ) {
                 alert("체크인/아웃을 확인하세요.");
                 return false;
            }
         
         var mr_CheckIn = $("#mr_CheckIn").val();     
         var mr_CheckOut = $("#mr_CheckOut").val();
         var mr_Book_Per = $("#mr_Book_Per").val();
         var mr_Book_Pw = $("#mr_Book_Pw").val();
         var m_name = "${mInfo.m_name}";
         var m_no = "${mInfo.m_no}";
            
         $.ajax({
            type : "POST",
            url : "mreservation",
            data : JSON.stringify({"mr_CheckIn" : mr_CheckIn,
                                   "mr_CheckOut":mr_CheckOut,
                                   "mr_no": mr_no,
                                   "mr_Book_Per":mr_Book_Per,
