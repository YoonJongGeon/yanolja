      ...
      <div class="container">
         <div class="row">
            <div class="title-inner">
               ...
               <div class="title">
               <c:choose>
	               <c:when test="${type eq 'hseoul'}" >
	                  <h2>서울 호텔 리스트</h2>               
	               </c:when>
	               <c:when test="${type eq 'hkangwon'}" >
	                  <h2>강원 호텔 리스트</h2>   
	               </c:when>
	               
	               <c:when test="${type eq 'hbusan'}" >
	                  <h2>부산 호텔 리스트</h2>   
	               </c:when>
	               
	               <c:when test="${type eq 'hjeju'}" >
	                  <h2>제주 호텔 리스트</h2>   
	               </c:when>
               </c:choose>
         ...
         $.getJSON(
            ...
            function(data){
               ...
               for(var i = 0; i < data.length; i++) {
                  ...
                  str += '';
                  str += '<a href="${pageContext.request.contextPath}/hotel/detail?hno=' + data[i].h_no + '">';
                  str += '<img src="${pageContext.request.contextPath}/resources/images/hotel/' + data[i].h_no + '.jpg"></a>';
                  str += '</div>';
                  str += '<div class="content-inner">';
                  str += '<a href="${pageContext.request.contextPath}/hotel/detail?hno=' + data[i].h_no + '"><h2>' + data[i].h_name + '</h2></a>';
                  str += '<p>숙박:<small>15:00~</small></p>';
                  str += '<h4><strong>' + comma(data[i].price) + '원 </strong></h4>';
                  str += '</div>';
                  str += '<div class="like-inner">';
                  str += '<span>'+ data[i].h_locabasic + '&nbsp;' + data[i].h_locadetail + '</span>';
                  str += '</div>';
               } // for 끝
    	         ...
    	         $.getJSON(
    	            ...
    	            function(data){
    	               ...
    	               for(var i = 0; i < data.length; i++) {
    	                  ...
    	                  str1 += '';
    	                  str1 += '<a href="${pageContext.request.contextPath}/hotel/detail?hno=' + data[i].h_no + '">';
    	                  str1 += '<img src="${pageContext.request.contextPath}/resources/images/hotel/'+ data[i].h_no +'.jpg"></a>';
    	                  str1 += '</div>';
    	                  str1 += '<div class="content-inner">';
    	                  str1 += '<a href="${pageContext.request.contextPath}/hotel/detail?hno=' + data[i].h_no + '"><h2>' + data[i].h_name + '</h2></a>';
    	                  str1 += '<p>숙박:<small>15:00~</small></p>';
    	                  str1 += '<h4><strong>' + comma(data[i].price) + '원 </strong></h4>';
    	                  str1 += '</div>';
    	                  str1 += '<div class="like-inner">';
    	                  str1 += '<span>'+ data[i].h_locabasic + '&nbsp;' + data[i].h_locadetail + '</span>';
    	                  str1 += '</div>';
    	               } // for 끝
