...
<body>
		...
		<div class="container">
			<div class="row">
				<div class="title-inner">
					...
					<!--제목영역-->
					<div class="title">
						<img src="${pageContext.request.contextPath}/resources/images/hotel/${hInfo.h_no }.jpg" width="70%">
						<h2>${hInfo.h_name }</h2>
						<p>${hInfo.h_locabasic}${hInfo.h_locadetail }</p>
					</div>
				</div>

				<!--반복-->
				<c:forEach var="vo" items="${hInfo.roominfo }">
					<div class="col-xs-12 col-sm-8 section-inner">
						<div id="contentDiv">
							<div class="image-inner">

								<!-- 이미지영역 -->
								<a href="${vo.hr_no }" data-toggle="modal" data-target="#myModal">
								<img src="${pageContext.request.contextPath}/resources/images/hotel/${hInfo.h_no }-${vo.hr_no }.jpg"></a>
							</div>
							<div class="content-inner">

								<!--내용영역-->
								<a href="#" data-toggle="modal" data-target="#myModal"><h2>${vo.hr_name }</h2></a>
								<p class="ss">기준2인 / 최대${vo.h_per }인</p>
								<br>
								<div class="p-content">
									<div class="p-left">
										<p>
									...
									<div class="p-rigth">
										<h4>
											<strong><fmt:formatNumber>${vo.h_price }</fmt:formatNumber>원</strong>
										</h4>
										<button name="${vo.hr_no }" id="${hInfo.h_no }" type="button" class="tribtn" data-toggle="modal" data-target="#myModal">예약</button>
									</div>
								</div>
							</div>
						</div>
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">${hInfo.h_name }</h4>
					</div>
					<div class="modal-body">
						<img src="${pageContext.request.contextPath}/resources/images/hotel/${hInfo.h_no }.jpg" class="innimage">
						<div class="InOut">
							<span class="checkIn"><small>체크인</small></span>
							<span class="checkOut"><small>체크아웃</small></span>
							<p>
								<input type="text" id="hr_CheckIn" size="25" onChange="handleChange()">
								<input type="text" id="hr_CheckOut" size="25" onChange="handleChange()">
							</p>
						</div>
						<div id="AllPerson">
							<div id="pperson">
								<p id="person">인원수</p>
								<select id="hr_Book_Per">
									<option>1명</option>
									<option>2명</option>
									<option>3명</option>
									<option>4명</option>
							...
							<input type="password" id="hr_Book_Pw">
						</div>
						<div class="room">
							<div class="title">
								<h4>
    ...
    <script>
		$(function() {
			...
			var from = $("#hr_CheckIn").datepicker(optionFrom).on("change", function() {
						to.datepicker("option", "minDate", getDate(this));
					});
			var optionTo = option;
			optionTo.altField = "#alternateTo";
			// 종료일이 선택이 되면 시작일은 시작일 보다 앞을 선택할 수 없다.
			var to = $("#hr_CheckOut").datepicker(optionTo).on("change", function() {
		...
		$(".bookbtn").click(function() {
			console.log("체크인 " + $("#hr_CheckIn").val());
			console.log("체크아웃 " + $("#hr_CheckOut").val());
		})
	</script>
    ...
    <script type="text/javascript">
    	...
    	$(document).ready(function() {
    		...
    		function add() {
				...
				$.ajax({
					type:"POST", 
					url:"../reply/hotel_input_reply",
					data:JSON.stringify({"h_no":h_no,"rating":idx, "reply":reply, "replyId":replyId}),//전송 데이터
	...
	function handleChange() {
		$.getJSON(
	    	'getPrice/' + hr_no + '/' + hno, // 요청보낼 주소
	        function(data){
	        	var realmoney = data;
	            var date_out = new Date($("#hr_CheckOut").val());
	            var date_in = new Date($("#hr_CheckIn").val());
	...
	/* 예약 함수 */
	function bookReserv(hr_no) {
		$(".bookbtn").click(function(){
			var bookpw = $("#hr_Book_Pw").val();
			...
			var hr_CheckIn = $("#hr_CheckIn").val();     
			var hr_CheckOut = $("#hr_CheckOut").val();
			var hr_Book_Per = $("#hr_Book_Per").val();
			var hr_Book_Pw = $("#hr_Book_Pw").val();
			var h_name = "${hInfo.h_name}";
			var h_no = "${hInfo.h_no}";
			   
			$.ajax({
				type : "POST",
				url : "hreservation",
				data : JSON.stringify({"hr_CheckIn" : hr_CheckIn,
				                       "hr_CheckOut":hr_CheckOut,
				                       "hr_no": hr_no,
				                       "hr_Book_Per":hr_Book_Per,
