      ...
      <div class="container">
         <div class="row">
            <div class="title-inner">
               ...
               <div class="title">
               <c:choose>
	               <c:when test="${type eq 'gseoul'}" >
	                  <h2>서울 게스트하우스 리스트</h2>               
	               </c:when>
	               <c:when test="${type eq 'gkangwon'}" >
	                  <h2>강원 게스트하우스 리스트</h2>   
	               </c:when>
	               
	               <c:when test="${type eq 'gbusan'}" >
	                  <h2>부산 게스트하우스 리스트</h2>   
	               </c:when>
	               
	               <c:when test="${type eq 'gjeju'}" >
	                  <h2>제주 게스트하우스 리스트</h2>   
	               </c:when>
               </c:choose>
               ...
               <form>
                  ...
                  <div class="search-wrap">
                     <span>${getTotal }개의 검색결과</span>
                     <button type="button" class="btn btn-info search-btn" id="searchBtn">검색</button>

                     <input type="text" class="form-control search-input" id="sName" placeholder="Search">
                  </div>
               </form>
            </div>
         ...
         $.getJSON(
            ...
            function(data){
               ...
               for(var i = 0; i < data.length; i++) {
                  ...
                  str += '';
                  str += '<a href="${pageContext.request.contextPath}/guesthouse/detail?gno=' + data[i].g_no + '">';
                  str += '<img src="${pageContext.request.contextPath}/resources/images/guesthouse/' + data[i].g_no + '.jpg"></a>';
                  str += '</div>';
                  str += '<div class="content-inner">';
                  str += '<a href="${pageContext.request.contextPath}/guesthouse/detail?gno=' + data[i].g_no + '"><h2>' + data[i].g_name + '</h2></a>';
                  str += '<p>숙박:<small>15:00~</small></p>';
                  str += '<h4><strong>' + comma(data[i].price) + '원 </strong></h4>';
                  str += '</div>';
                  str += '<div class="like-inner">';
                  str += '<span>'+ data[i].g_locabasic + '&nbsp;' + data[i].g_locadetail + '</span>';
                  str += '</div>';
               } // for 끝
    	         ...
    	         $.getJSON(
    	            ...
    	            function(data){
    	               ...
    	               for(var i = 0; i < data.length; i++) {
    	                  ...
    	                  str1 += '';
    	                  str1 += '<a href="${pageContext.request.contextPath}/guesthouse/detail?gno=' + data[i].g_no + '">';
    	                  str1 += '<img src="${pageContext.request.contextPath}/resources/images/guesthouse/'+ data[i].g_no +'.jpg"></a>';
    	                  str1 += '</div>';
    	                  str1 += '<div class="content-inner">';
    	                  str1 += '<a href="${pageContext.request.contextPath}/guesthouse/detail?gno=' + data[i].g_no + '"><h2>' + data[i].g_name + '</h2></a>';
    	                  str1 += '<p>숙박:<small>15:00~</small></p>';
    	                  str1 += '<h4><strong>' + comma(data[i].price) + '원 </strong></h4>';
    	                  str1 += '</div>';
    	                  str1 += '<div class="like-inner">';
    	                  str1 += '<span>'+ data[i].g_locabasic + '&nbsp;' + data[i].g_locadetail + '</span>';
    	                  str1 += '</div>';
    	               } // for 끝
