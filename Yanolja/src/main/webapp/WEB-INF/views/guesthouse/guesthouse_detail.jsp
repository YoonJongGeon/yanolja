...
<body>
		...
		<div class="container">
			<div class="row">
				<div class="title-inner">
					...
					<!--제목영역-->
					<div class="title">
						<img src="${pageContext.request.contextPath}/resources/images/guesthouse/${gInfo.g_no }.jpg" width="70%">
						<h2>${gInfo.g_name }</h2>
						<p>${gInfo.g_locabasic}${gInfo.g_locadetail }</p>
					</div>
				</div>

				<!--반복-->
				<c:forEach var="vo" items="${gInfo.roominfo }">
					<div class="col-xs-12 col-sm-8 section-inner">
						<div id="contentDiv">
							<div class="image-inner">

								<!-- 이미지영역 -->
								<a href="${vo.gr_no }" data-toggle="modal" data-target="#myModal">
								<img src="${pageContext.request.contextPath}/resources/images/guesthouse/${gInfo.g_no }-${vo.gr_no }.jpg"></a>
							</div>
							<div class="content-inner">

								<!--내용영역-->
								<a href="#" data-toggle="modal" data-target="#myModal"><h2>${vo.gr_name }</h2></a>
								<p class="ss">기준2인 / 최대${vo.g_per }인</p>
								<br>
								<div class="p-content">
									<div class="p-left">
										<p>
									...
									<div class="p-rigth">
										<h4>
											<strong><fmt:formatNumber>${vo.g_price }</fmt:formatNumber>원</strong>
										</h4>
										<button name="${vo.gr_no }" id="${gInfo.g_no }" type="button" class="tribtn" data-toggle="modal" data-target="#myModal">예약</button>
									</div>
								</div>
							</div>
						</div>
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">${gInfo.g_name }</h4>
					</div>
					<div class="modal-body">
						<img src="${pageContext.request.contextPath}/resources/images/guesthouse/${gInfo.g_no }.jpg" class="innimage">
						<div class="InOut">
							<span class="checkIn"><small>체크인</small></span>
							<span class="checkOut"><small>체크아웃</small></span>
							<p>
								<input type="text" id="gr_CheckIn" size="25" onChange="handleChange()">
								<input type="text" id="gr_CheckOut" size="25" onChange="handleChange()">
							</p>
						</div>
						<div id="AllPerson">
							<div id="pperson">
								<p id="person">인원수</p>
								<select id="gr_Book_Per">
									<option>1명</option>
									<option>2명</option>
									<option>3명</option>
									<option>4명</option>
							...
							<input type="password" id="gr_Book_Pw">
						</div>
						<div class="room">
							<div class="title">
								<h4>
    ...
    <script>
		$(function() {
			...
			var from = $("#gr_CheckIn").datepicker(optionFrom).on("change", function() {
						to.datepicker("option", "minDate", getDate(this));
					});
			var optionTo = option;
			optionTo.altField = "#alternateTo";
			// 종료일이 선택이 되면 시작일은 시작일 보다 앞을 선택할 수 없다.
			var to = $("#gr_CheckOut").datepicker(optionTo).on("change", function() {
		...
		$(".bookbtn").click(function() {
			console.log("체크인 " + $("#gr_CheckIn").val());
			console.log("체크아웃 " + $("#gr_CheckOut").val());
		})
	</script>
    ...
    <script type="text/javascript">
    	...
    	$(document).ready(function() {
    		...
    		function add() {
				...
				$.ajax({
					type:"POST", 
					url:"../reply/guesthouse_input_reply",
					data:JSON.stringify({"g_no":g_no,"rating":idx, "reply":reply, "replyId":replyId}),//전송 데이터
    		...
    		function getList(reset) {
    			var g_no = '${gno}';
				$.getJSON(
					"../reply/guesthouse_list_reply/" + g_no, 
					function(data) {
	...
	function handleChange() {
		$.getJSON(
	    	'getPrice/' + gr_no + '/' + gno, // 요청보낼 주소
	        function(data){
	        	var realmoney = data;
	            var date_out = new Date($("#gr_CheckOut").val());
	            var date_in = new Date($("#gr_CheckIn").val());
	...
	/* 예약 함수 */
	function bookReserv(gr_no) {
		$(".bookbtn").click(function(){
			var bookpw = $("#gr_Book_Pw").val();
			...
			var gr_CheckIn = $("#gr_CheckIn").val();     
			var gr_CheckOut = $("#gr_CheckOut").val();
			var gr_Book_Per = $("#gr_Book_Per").val();
			var gr_Book_Pw = $("#gr_Book_Pw").val();
			var g_name = "${gInfo.g_name}";
			var g_no = "${gInfo.g_no}";
			   
			$.ajax({
				type : "POST",
				url : "greservation",
				data : JSON.stringify({"gr_CheckIn" : gr_CheckIn,
				                       "gr_CheckOut": gr_CheckOut,
				                       "gr_no": gr_no,
				                       "gr_Book_Per": gr_Book_Per,
